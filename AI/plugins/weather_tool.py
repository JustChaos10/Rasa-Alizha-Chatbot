...
class WeatherTool(BaseTool):
        ...
        try:
            ...
            if days_offset == 0:
                ...
                if original_language:
                    weather_data["original_language"] = original_language
            else:
                # Forecast weather
    ...
    def format_response(self, result: Dict[str, Any]) -> str:
        ...
        is_arabic_request = original_language == "ar"

        # For Arabic requests, avoid emojis/icons (spec requirement).
        # The response text will be translated EN→AR later in `app.py`.
        if is_arabic_request:
            ...
            if is_forecast:
                ...
                lines.append(f"**Weather Forecast for {location}**")
                if forecast_date:
                    lines.append(f"*{forecast_date}* ({when_text})")
            else:
                if when_text == "today":
                    lines.append(f"**Today's Weather in {location}**")
                else:
                    lines.append(f"**Current Weather in {location}**")
            ...
            if temp is not None:
                ...
                lines.append(f"**Temperature:** {temp_rounded}°C ({temp_f}°F)")
                if feels_like is not None and abs(feels_like - temp) > 2:
                    lines.append(f"   *Feels like* {round(feels_like)}°C")
            ...
            if humidity is not None:
                humidity_indicator = strip_emojis(self._get_humidity_indicator(humidity))
                lines.append(f"   **Humidity:** {humidity}% {humidity_indicator}")

            if wind is not None:
                wind_desc = strip_emojis(self._get_wind_description(wind))
            ...
            if visibility is not None:
                ...
                lines.append(f"   **Visibility:** {vis_km} km")
            ...
            tip = strip_emojis(self._get_weather_tip(temp, humidity, (data.get("condition", "") or "").lower(), wind))
            if tip:
                lines.append("")
