...
class ChatTool(BaseTool):
    ...
    def __init__(self):
        ...
        if IMAGE_SERVICE_AVAILABLE:
            try:
                ...
                logger.warning(f"⚠️ ChatTool: Could not initialize ImageSearchService: {_redact_secrets(str(e))}")
    ...
    async def _web_search(self, query: str, max_results: int = 5) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Web search failed for '{query}': {_redact_secrets(str(e))}")
            return None
    ...
    def _fetch_image(self, topic: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch image for '{topic}': {_redact_secrets(str(e))}")
            return None
...
JSON array:"""
            ...
            if force_gemini or not self._api_key:
                from shared_utils import _gemini_generate_text
                content = await asyncio.to_thread(_gemini_generate_text, prompt, 0.5, 300)
                json_match = re.search(r'\[.*\]', content, re.DOTALL)
                if json_match:
                    questions = json.loads(json_match.group())
                    valid_questions = []
            ...
            logger.warning(f"Failed to generate related questions: {_redact_secrets(str(e))}")
    ...
    async def execute(
        ...
        **kwargs
        ...
        async def _call_gemini(prompt: str, temperature: float, max_tokens: int) -> str:
            from shared_utils import _gemini_generate_text
            return await asyncio.to_thread(_gemini_generate_text, prompt, temperature, max_tokens)
        ...
        try:
            ...
            if force_bedrock:
                ...
                with trace_llm_call(
                    ...
                    metadata={"source": "chat_tool.execute", "provider": "bedrock"}
                ) as trace:
                    ...
                    trace.update(
                        output=assistant_message[:500],
                        metadata={"success": bool(assistant_message), "response_length": len(assistant_message)}
                    )

            elif force_gemini or not self._api_key:
                ...
                with trace_llm_call(
                    ...
                    model_parameters={"temperature": 0.7, "max_tokens": 1000},
                    metadata={"source": "chat_tool.execute", "provider": "gemini"}
                ) as trace:
                    assistant_message = await _call_gemini(prompt, temperature=0.7, max_tokens=1000)
                    trace.update(
                        output=assistant_message[:500],
                        metadata={"success": bool(assistant_message), "response_length": len(assistant_message)}
                    )
            ...
            safe_error = _redact_secrets(str(e))
            logger.error(f"Chat API error: {safe_error}")
            if force_gemini:
                return {"success": False, "error": f"Failed to get response: {safe_error}"}
            ...
            try:
                ...
                return {"success": False, "error": f"Failed to get response: {_redact_secrets(str(ge))}"}
        except Exception as e:
            safe_error = _redact_secrets(str(e))
            logger.error(f"Unexpected chat error: {safe_error}")
            return {"success": False, "error": f"Unexpected error: {safe_error}"}
