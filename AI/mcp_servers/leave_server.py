...
async def extract_leave_entities(query: str) -> Dict[str, Any]:
    ...
    use_bedrock = prefer_env in {"aws", "bedrock"}

    if not GROQ_API_KEY and not use_bedrock:
...
6. Return ONLY the JSON object, nothing else"""
    ...
    model_str = (
        f"bedrock/{os.getenv('BEDROCK_MODEL_ID','meta.llama3-1-8b-instruct-v1:0')}"
        if use_bedrock
        else f"groq/{ENTITY_MODEL}"
    )
