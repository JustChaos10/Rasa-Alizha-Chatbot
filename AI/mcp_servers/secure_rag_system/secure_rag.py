...
class RBACService:
    ...
    def check_access(self, user: User, query: str) -> AccessResult:
        ...
        # Check if user is asking for employee-specific personal/compensation data without proper access.
        # This must deny even if the vector search returns no relevant docs, to avoid probing/leakage via retrieval gaps.
        sensitive_employee_markers_en = [
            "akash",
            "arpan",
            "salary",
            "compensation",
        ...
        is_sensitive_employee_query = any(kw in query_lower for kw in sensitive_employee_markers_en) or any(
            kw in query for kw in sensitive_employee_markers_ar
        )

        if is_sensitive_employee_query and user.role not in [UserRole.ADMIN, UserRole.HR_ADMIN]:
            return AccessResult(
                allowed=False,
                user=user,
                reason="Access denied: Employee information is restricted. Only administrators and HR can view this data.",
                filtered_documents=[],
                access_level=user.access_level,
            )
...
class LLMService:
    ...
    def generate(self, query: str, context: str) -> str:
        ...
        prefer_env = (os.environ.get("PREFER_LLM_PROVIDER") or os.environ.get("LLM_PROVIDER") or "").strip().lower()
...
class SecureRAGPipeline:
    ...
    def process_query(self, query: str, user_id: str, db_role: str = None) -> SecureRAGResult:
        ...
        context = "\n\n".join(context_parts)
        retrieval_context = [doc.get("text", "") for doc in access_result.filtered_documents]
