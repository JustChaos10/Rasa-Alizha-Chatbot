...
class VectorStoreRetriever:
    ...
    def retrieve(self, query: str) -> Tuple[str, List[str], List[str]]:
        """Retrieve relevant documents for a query. Returns (context_string, chunks, sources)."""
        ...
        try:
            ...
            context_string = "\n\n---\n\n".join(context_parts)
            return context_string, chunks, sources
...
class RAGSystem:
    ...
    def __init__(self, config):
        ...
        self.charts_dir = str((self._web_charts_root / "kb").resolve())
...
Respond with ONLY: YES or NO"""
    ...
    async def generate_answer(self, query: str, state_id: str) -> Tuple[str, Optional[str], List[str], List[str]]:
        """
        ...
        Returns:
            Tuple of (answer_text, chart_path or None, retrieval_chunks, sources)
        """
        ...
        try:
            ...
            context, retrieval_chunks, sources = self.retriever.retrieve(query)
            
            if "not available" in context.lower() or "error" in context.lower():
                return context, None, retrieval_chunks, sources
            ...
            if not response.success:
                ...
                return self._create_fallback_answer(context), None, retrieval_chunks, sources
            ...
            return answer, chart_path, retrieval_chunks, sources
...
async def knowledgebase_query(query: str, state_id: str = "default") -> str:
    ...
    try:
        ...
        answer, viz_path, retrieval_chunks, sources = await rag_system.generate_answer(query, state_id)
        response = {"answer": answer, "chart": viz_path, "retrieval_context": retrieval_chunks, "sources": sources}
