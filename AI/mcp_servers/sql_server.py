...
# Import SQL security validator
from sql_security import get_sql_validator
...
class SQLServer:
    ...
    def __init__(self, config):
        ...
        charts_root = (os.getenv("WEB_CHARTS_DIR") or os.getenv("CHARTS_DIR") or "").strip()
        if charts_root:
            self._web_charts_root = Path(charts_root)
        else:
            # Default to the Web tier charts directory (repo_root/Web/charts)
            self._web_charts_root = Path(__file__).resolve().parents[2] / "Web" / "charts"
        self.charts_dir = str((self._web_charts_root / "sql").resolve())
...
JSON Response:"""
    ...
    async def execute_sql(self, sql: str, user_id: str = "system") -> str:
        """
        Execute SQL query with security validation.
        
        SECURITY: All queries are validated before execution to prevent SQL injection.
        """
        # Validate query for security
        validator = get_sql_validator()
        is_valid, error_msg = validator.validate_query(sql, user_id)
        
        if not is_valid:
            logger.error(f"SQL query blocked: {error_msg}")
...
Respond with ONLY: YES or NO"""
    ...
    async def process_query(self, question: str, state_id: str, user_id: str = "system") -> Tuple[str, Optional[str]]:
        """
        Process a natural language query end-to-end.
        
        Sequential flow:
        1. Generate SQL (LLM call #1)
        2. Execute SQL with security validation
        3. Analyze results (LLM call #2) → returns {summary, visualization}
        4. If visualization config present, render chart (no LLM)
        
        Args:
            question: Natural language query
            state_id: Session identifier
            user_id: User identifier for audit logging
        
        Returns:
            Tuple of (response_text, chart_path or None)
        """
        logger.info(f"Processing SQL query: {question[:100]}... [state_id={state_id}, user={user_id}]")
        ...
        try:
            ...
            # Step 2: Execute SQL (security validation handled in execute_sql)
            result = await self.execute_sql(sql, user_id)
            ...
            if viz_config:
                ...
                if chart_path:
                    try:
                        rel = Path(chart_path).resolve().relative_to(self._web_charts_root.resolve())
                        chart_path = f"charts/{rel.as_posix()}"
                    except Exception:
            ...
            return f"Security error: {str(e)}", None
        except Exception as e:
...
@mcp.tool()
async def sql_query(query: str, state_id: str = "default", user_id: str = "system") -> str:
    """
    ...
    Args:
        ...
        user_id: User identifier for security audit logging
        
    Returns:
        JSON string with answer and optional chart path
    """
    try:
        logger.info(f"SQL query received: '{query}' [state_id={state_id}, user={user_id}]")
        answer, viz_path = await sql_server.process_query(query, state_id, user_id)
