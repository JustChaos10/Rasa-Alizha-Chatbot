...
def _extract_json_object(text: str) -> Optional[Dict[str, Any]]:
    ...
    Bedrock/Llama sometimes wraps JSON with extra prose; this keeps routing resilient.
    """
    if not text:
        return None
...
class LLMRouter:
    ...
    def __init__(self, config: Optional[RouterConfig] = None):
        ...
        self._gemini_vertex_enabled = _gemini_vertex_enabled()
        ...
        if not self.config.bedrock_model_id:
            self.config.bedrock_model_id = (os.getenv("BEDROCK_MODEL_ID") or "meta.llama3-1-8b-instruct-v1:0").strip()
        
        prefer_env = (os.getenv("PREFER_LLM_PROVIDER") or os.getenv("LLM_PROVIDER") or "").strip().lower()
        bedrock_preferred = prefer_env in {"aws", "bedrock"}
        bedrock_configured = bool(self.config.bedrock_region and self.config.bedrock_model_id)

        if not self.config.groq_api_key and not self.config.gemini_api_key and not self._gemini_vertex_enabled and not (bedrock_preferred and bedrock_configured):
        ...
        providers = []
        if bedrock_preferred and bedrock_configured:
    ...
    async def initialize(self) -> None:
        ...
        if self.config.enable_mcp:
            try:
                ...
                logger.warning(f"⚠️ MCP initialization failed: {_redact_secrets(str(e))}")
    ...
    async def _call_gemini(
        ...
        Returns LLMResult with content and usage data.
        """
        if not self.config.gemini_api_key and not self._gemini_vertex_enabled:
            raise ValueError(
                "Gemini not configured (set GEMINI_API_KEY/GOOGLE_API_KEY or GOOGLE_APPLICATION_CREDENTIALS + GEMINI_PROVIDER=vertex)"
            )
        ...
        if self._gemini_vertex_enabled:
            ...
            def _run_sync():
                ...
                return client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        temperature=temperature if temperature is not None else self.config.routing_temperature,
                        max_output_tokens=max_tokens or self.config.routing_max_tokens,
                    ),
                )
    ...
    async def _call_bedrock(
        ...
        """
        Call AWS Bedrock (Meta Llama 3.1 8B Instruct) using boto3 InvokeModel.
        """
        ...
        try:
            content = await asyncio.to_thread(
                ...
                max_tokens=int(max_tokens or self.config.routing_max_tokens),
                temperature=float(temperature if temperature is not None else self.config.routing_temperature),
            )
        except BedrockNotConfigured as e:
            raise ValueError(str(e)) from e

        return LLMResult(content=content, usage=None, provider="bedrock", model=self.config.bedrock_model_id)
    ...
    async def _call_llm(
        ...
        bedrock_available = bool(self.config.bedrock_region and self.config.bedrock_model_id)
        groq_available = bool(self.config.groq_api_key) and not force_gemini and not force_bedrock
        gemini_available = (bool(self.config.gemini_api_key) or self._gemini_vertex_enabled) and not force_bedrock
        
        if force_bedrock and not bedrock_available:
            raise ValueError("Bedrock preferred but not configured (set AWS_REGION and BEDROCK_MODEL_ID)")

        if not bedrock_available and not groq_available and not gemini_available:
            raise ValueError("No LLM providers configured (need Bedrock (AWS_REGION), GROQ_API_KEY, or Gemini configuration)")

        if force_bedrock and bedrock_available:
            providers = ["bedrock"]
        else:
                ...
                try:
                    ...
                    with trace_llm_call(
                        ...
                        model=(
                            ...
                            if provider == "groq"
                            else (f"{provider}/{self.config.bedrock_model_id}" if provider == "bedrock" else f"{provider}/{self.config.gemini_model}")
                        ),
                        ...
                        if provider == "groq":
                            ...
                            result = await self._call_bedrock(messages, temperature, max_tokens)
                        else:  # gemini
                    ...
                    if status in {401, 403}:
                        logger.warning(f"ƒsÿ‹,? {provider.upper()} unauthorized ({status}), trying fallback...")
                        log_llm_event("provider-unauthorized", {"provider": provider, "status": status}, level="ERROR")
                        if provider == "groq":
                            self._groq_failures += 1
                        else:
                            self._gemini_failures += 1
                    ...
                    logger.warning(f"⚠️ {provider.upper()} failed: {_redact_secrets(str(e))}, trying fallback...")
...
- "Hello there" -> {{"tool": "chat", "params": {{"message": "Hello there"}}, "confidence": 1.0, "reasoning": "greeting - general conversation"}}"""""
    ...
    async def route(self, query: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if self._mcp_host and self._mcp_host.connected_servers > 0:
            try:
                ...
                logger.warning(f"MCP tool search failed: {_redact_secrets(str(e))}")
        ...
        try:
            ...
            recovered = _extract_json_object(response)
            if isinstance(recovered, dict):
                return recovered

            logger.error(f"Failed to parse LLM routing response: {_redact_secrets(str(e))}")
            return {
                "tool": "chat",
                "params": {"message": query},
                "confidence": 0.5,
            ...
            logger.error(f"Routing error: {_redact_secrets(str(e))}")
            return {
                "tool": "chat",
                "params": {"message": query},
                "confidence": 0.5,
    ...
    async def execute_tool(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "." in tool_name and self._mcp_host:
            ...
            with tool_logger.tool_call(
                ...
                try:
                    ...
                    logger.error(f"MCP tool execution error: {_redact_secrets(str(e))}")
                    log.success = False
                    log.error = str(e)
                    return {
                        "success": False,
        ...
        with tool_logger.tool_call(
            ...
            try:
                ...
                logger.error(f"Tool execution error: {_redact_secrets(str(e))}")
                log.success = False
                log.error = str(e)
                return {
                    "success": False,
    ...
    async def execute_with_code(
        ...
        try:
            ...
            logger.error(f"Code execution error: {_redact_secrets(str(e))}")
            return {
                "success": False,
                "error": str(e)
            }
...
```"""
    ...
    async def _route_and_execute_code_first(
        ...
        # If an MCP tool is an extremely strong match, call it directly.
        # This avoids LLM misrouting when tool search has high confidence.
        if self._mcp_host and self._mcp_host.connected_servers > 0:
            try:
                direct = await self._mcp_host.search_tools(query, max_results=1)
                ql = (query or "").strip().lower()
                looks_like_question = (
                    ql.endswith("?")
                    or ql.startswith(("what ", "who ", "where ", "when ", "why ", "how ", "which "))
                    or ql.startswith(("can ", "is ", "are ", "do ", "does ", "did ", "should ", "could ", "would "))
                )
                if direct and looks_like_question and direct[0].relevance_score >= 0.9:
                    top = direct[0]
                    full_name = f"{top.server_name}.{top.tool.name}"
                    args: Dict[str, Any] = {}
                    schema = getattr(top.tool, "input_schema", {}) or {}
                    ...
                    if "query" in props:
                        ...
                        lowered = q.lower()
                        topic = q
                        if "brochure" in lowered and " on " in lowered:
                            topic = q.split(" on ", 1)[1].strip()
                        args["topic"] = topic

                    if "state_id" in props:
                        args["state_id"] = (context or {}).get("_sender_id", "default")
                    if "user_id" in props:
                        args["user_id"] = (context or {}).get("employee_id", "guest")
                    if "user_role" in props:
                        args["user_role"] = (context or {}).get("role", "user")
                    if "employee_id" in props and "employee_id" not in args:
                        args["employee_id"] = (context or {}).get("employee_id", "1")

                    tool_result = await self._mcp_host.call_tool(full_name, args)
                    payload = tool_result.get("data") if tool_result.get("success") else tool_result

                    response_text = ""
                    if isinstance(payload, dict):
                        response_text = payload.get("message") or payload.get("answer") or payload.get("response") or ""
        ...
        # Get minimal tool list (local tools + query-relevant MCP tools)
        tool_list = await self._get_minimal_tool_list_for_query(query)
        ...
        # Restrict which MCP servers the generated code may call.
        # This prevents common misrouting/hallucination (e.g., secure_rag for SQL/budget queries).
        allowed_servers: Optional[List[str]] = None
        if self._mcp_host and self._mcp_host.connected_servers > 0:
            try:
                results = await self._mcp_host.search_tools(query, max_results=8)
                if results:
                    best = max(r.relevance_score for r in results) or 0.0
                    min_score = best * 0.75
                    allowed_servers = sorted({r.server_name for r in results if r.relevance_score >= min_score})
            except Exception:
                        ...
                        if tool:
                            if hasattr(tool, 'format_response'):
                                try:
                                    ...
                                    logger.warning(f"Tool format_response failed for {tool_name}: {_redact_secrets(str(e))}")
                            else:
        ...
        if result.success:
            ...
            # If the code-first path failed before calling any tools, fall back to chat.
            err = str(result.error or "")
            if not result.tool_calls and ("Code validation failed" in err or "Syntax error" in err):
                history_str = context.get("_conversation_history", "") if context else ""
                chat_result = await self.chat(query, history_str)
                response_text = chat_result.get("data", "") if isinstance(chat_result, dict) else str(chat_result)
                return {
                    ...
                    "result": chat_result if isinstance(chat_result, dict) else {"success": True, "data": chat_result},
                    "response": response_text,
                    "mode": "code_first_error_fallback",
                }
    ...
    async def _get_minimal_tool_list_for_query(self, query: str) -> str:
        """
        Build a minimal tool list for code generation.

        Always includes local plugins.
        Includes only the most relevant MCP tools for the current query to reduce
        misrouting (e.g., avoid choosing secure_rag for SQL/budget questions).
        """
        lines = ["[Local Plugins]"]
        ...
        if self._mcp_host and self._mcp_host.connected_servers > 0:
            lines.append("\n[MCP Servers - Relevant]")
            try:
                results = await self._mcp_host.search_tools(query, max_results=8, detail_level="brief")
                if results:
                    best = max(r.relevance_score for r in results) or 0.0
                    min_score = best * 0.75
                    # Keep only strong matches, then keep at most one tool per server (prefer non-health/schema)
                    strong = [r for r in results if r.relevance_score >= min_score]
                    by_server = {}
                    for r in strong:
                        existing = by_server.get(r.server_name)
                        if not existing:
                            by_server[r.server_name] = r
                            continue
                        name = (r.tool.name or "").lower()
                        existing_name = (existing.tool.name or "").lower()
                        bad = any(k in name for k in ("health", "schema"))
                        existing_bad = any(k in existing_name for k in ("health", "schema"))
                        if existing_bad and not bad:
                            by_server[r.server_name] = r

                    for r in by_server.values():
                        full_name = f"{r.server_name}.{r.tool.name}"
                        desc = (r.tool.description or "")[:80]
    ...
    def _get_tool_metadata_for_tools(self, tool_names: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for tool_name in tool_names:
            ...
            if self._mcp_host and "." in tool_name:
                ...
                # Local plugin metadata (ToolSchema.system_instruction / code_example)
                try:
                    schema = self.registry.get_schema(tool_name) if self.registry else None
                    if schema:
                        if getattr(schema, "system_instruction", None):
                            system_instructions.append(f"# {tool_name}:\n{schema.system_instruction}")
                        if getattr(schema, "code_example", None):
                            code_examples.append(f"# {tool_name}:\n{schema.code_example}")
                except Exception:
                    pass
    ...
    async def _generate_tool_code(
        ...
        # =====================================================================
        matched_local_tools: List[str] = []
        strong_local_match = False
        preferred_local_hint: str = ""
        try:
            query_lower = (query or "").lower().strip()
            if query_lower:
                for local_tool in (self.registry.list_tools() if self.registry else []):
                    schema = self.registry.get_schema(local_tool) if self.registry else None
                    if not schema or not getattr(schema, "examples", None):
                        continue
                    for ex in schema.examples:
                        ex_lower = (ex or "").lower().strip()
                        if not ex_lower:
                            continue
                        if query_lower == ex_lower:
                            strong_local_match = True
                            matched_local_tools.append(local_tool)
                            # Generic action inference for tools that expose an `action` parameter
                            try:
                                action_choices = set()
                                props = (schema.parameters or {}).get("properties", {})
                                action_spec = props.get("action", {}) if isinstance(props, dict) else {}
                                for val in (action_spec.get("enum") or []):
                                    if isinstance(val, str):
                                        action_choices.add(val)
                                inferred_action = None
                                if any(k in query_lower for k in ("show", "view", "display")) and "show" in action_choices:
                                    inferred_action = "show"
                                elif any(k in query_lower for k in ("start", "collect", "generate", "create", "begin")) and "start" in action_choices:
                                    inferred_action = "start"
                                if inferred_action:
                                    preferred_local_hint = f"For this query, call {local_tool} with action='{inferred_action}'."
                            except Exception:
                                pass
                            break
                        if query_lower in ex_lower or ex_lower in query_lower:
                            matched_local_tools.append(local_tool)
                            break
        except Exception:
            matched_local_tools = []
        ...
        matched_mcp_tools = []
        # If we have a strong local match, avoid injecting MCP metadata that can bias routing away
        # from the intended local plugin (e.g., contact_form vs secure_rag for "Show my info").
        if not strong_local_match and self._mcp_host and self._mcp_host.connected_servers > 0:
            try:
                ...
                logger.warning(f"MCP tool search for metadata failed: {_redact_secrets(str(e))}")
        
        # =====================================================================
        # STEP 2: Get metadata only for matched tools (Teach phase)
        # =====================================================================
        dynamic_instructions, dynamic_examples = self._get_tool_metadata_for_tools(
            matched_local_tools + matched_mcp_tools
        )
        ...
        if preferred_local_hint:
            specific_tool_rules = (specific_tool_rules or "\nSPECIFIC TOOL RULES (for matched tools):") + "\n" + preferred_local_hint
...
"""
        ...
        include_secure_rag_note = any(t.startswith("secure_rag.") for t in matched_mcp_tools)
        secure_rag_note = ""
        if include_secure_rag_note:
            secure_rag_note = f"""
...
STRICT RULE: You MUST ONLY call tools that appear in AVAILABLE TOOLS above. Do not invent tool names.

{f"IMPORTANT: For 'secure_rag.secure_query', you MUST use the CURRENT USER INFO above.\nExample: result = await call_tool(\"secure_rag.secure_query\", {{\"query\": \"...\", \"user_id\": \"{user_id}\", \"user_role\": \"{user_role}\"}})\n" if include_secure_rag_note else ""}
...
14. IMPORTANT: Use secure_rag.secure_query for internal HR/company knowledge (employees, salaries, roles, policies) with RBAC using CURRENT USER INFO.
15. IMPORTANT: For collecting/showing the user's own contact details: use contact_form (NOT secure_rag).
16. IMPORTANT: For questions about uploaded files, documents, images, PDFs, summaries of uploaded content: Use "file" tool with action="followup" and question parameter
...
"""
        ...
        try:
            ...
            logger.error(f"Code generation failed: {_redact_secrets(str(e))}")
            return None
    ...
    async def chat(self, message: str, conversation_history: Optional[str] = None) -> Dict[str, Any]:
        ...
        if chat_tool:
            try:
                ...
                logger.error(f"ChatTool error: {_redact_secrets(str(e))}")
        ...
        try:
            ...
            logger.error(f"Chat error: {_redact_secrets(str(e))}")
