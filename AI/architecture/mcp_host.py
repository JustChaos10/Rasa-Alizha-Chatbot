...
class MCPHost:
    ...
    async def load_config(self, config_path: str) -> bool:
        ...
        """
        try:
            enabled_only = {s.strip() for s in (os.getenv("MCP_ENABLED_SERVERS") or "").split(",") if s.strip()}
            disabled = {s.strip() for s in (os.getenv("MCP_DISABLED_SERVERS") or "").split(",") if s.strip()}
            ...
            for name, server_config in servers.items():
                ...
                # Apply optional environment filters
                enabled_flag = server_config.get("enabled", True)
    ...
    async def call_tool(
        ...
        if "." not in full_name:
            return {
                "success": False,
                "error": f"Invalid tool name format: {full_name}. Use 'server.tool_name'",
            }
        ...
        if not client:
            ...
            if server_name in self._server_configs:
                if not await self.connect(server_name):
                    return {"success": False, "error": f"Failed to connect to server: {server_name}"}
                client = self._clients.get(server_name)
            else:
        ...
        # Defensive normalization: ensure the result always has a boolean success flag
